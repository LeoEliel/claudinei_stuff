...
import desafio.controller as controller
...
def criar_botao(app):
    ...
    btn_captura = ttk.Button(app, text='Inserir dados', style='Green.TButton', command=controller.capturar(nome.get(), sobrenome.get(), genero_var.get()))
    btn_captura.place(x=20, y=220, width=155, height=40)

    btn_pesquisar = ttk.Button(app, text='Pesquisar dados', style='Blue.TButton', command=model.mostrar_campo_pesquisa)
    btn_pesquisar.place(x=185, y=220, width=155, height=40)
    
    btn_atualizar = ttk.Button(app, text='Atualizar dados', style='Green.TButton', command=model.salvar_edicao)
    btn_atualizar.place(x=350, y=220, width=155, height=40)

    btn_apagar = ttk.Button(app, text='Apagar Registro', style='Red.TButton', command=lambda: model.excluir_registro(btn_captura, mensagem_var))
...
def criar_campo_pesquisa(app):
    ...
    campo_pesquisa.bind('<KeyRelease>', controller.filtrar_dados)
...
def criar_treeview():
    ...
    tree.bind("<Double-1>", controller.on_item_double_click)
